...
img = (np.arange(300)[:, np.newaxis] * np.arange(400)).astype('uint8')
img_warped = result.apply(img)

# Display the image, warped image, and valid source and target areas in one
top_row = np.concatenate((img, img_warped), axis=1)
bot_row = 255 * np.concatenate((source_area, target_area), axis=1).astype('uint8')
combined_img = np.concatenate((top_row, bot_row), axis=0)
